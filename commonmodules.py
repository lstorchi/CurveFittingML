...
def build_model_GP_2 (train_x, train_y, nuval = 5.0/2.0):
    #kernel = gp.kernels.ConstantKernel(1.0, (1e-5, 1e5))* gp.kernels.RBF(length_scale=1)
...
def build_model_GP_3D (train_x, train_y, nuval=5.0/2.0):
    #kernel = gp.kernels.ConstantKernel(1.0, (1e-5, 1e5))* gp.kernels.RBF(length_scale=1)
    kernel = 1.0 * gp.kernels.Matern(length_scale=1.0, nu=nuval)
    model = gp.GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=50, \
        normalize_y=False)
...
def buildmodel(modelshape, inputshape=3, lossf='mse', \
        optimizerf='adam', activationf='relu'):
    model = keras.Sequential()
    model.add(keras.layers.Input(shape=[inputshape]))

    for n in modelshape:
        model.add(keras.layers.Dense(units = n, activation = activationf))
